...
function showQueueInfo(message = '正在处理中...', progress = 0) {
    const queueInfo = document.getElementById('queue_info');
    const queueDetail = document.getElementById('queue_detail');
    const progressBar = document.getElementById('progress_bar');

    if (!queueInfo || !queueDetail || !progressBar) return;

    queueInfo.classList.remove('d-none');
    queueDetail.textContent = message;
    progressBar.style.display = 'block';
    progressBar.style.width = `${Math.max(0, Math.min(100, Number(progress) || 0))}%`;
    progressBar.setAttribute('aria-valuenow', String(Math.max(0, Math.min(100, Number(progress) || 0))));
}

function updateQueueInfo(message, progress) {
    const queueDetail = document.getElementById('queue_detail');
    const progressBar = document.getElementById('progress_bar');

    if (queueDetail && message) {
        queueDetail.textContent = message;
    }
    if (progressBar && progress !== undefined) {
        const cleanProgress = Math.max(0, Math.min(100, Number(progress) || 0));
        progressBar.style.display = 'block';
        progressBar.style.width = `${cleanProgress}%`;
        progressBar.setAttribute('aria-valuenow', String(cleanProgress));
    }
}
...
function showImagePreview(imageUrl) {
    ...
    if (!dropPlaceholder || !imagePreview || !previewImg || !uploadedFileInfo) return;
