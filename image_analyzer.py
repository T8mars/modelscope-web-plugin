...
from config import IMAGE_ANALYSIS_FALLBACK_MODELS, IMAGE_ANALYSIS_MODEL, MODELSCOPE_BASE_URL

def analyze_image(image_path, api_key):
    """调用 ModelScope OpenAI-compatible API 分析图片"""
    try:
        if not api_key:
            return False, "未配置图片反推 API Token，请设置 MODELSCOPE_API_KEY 或 OPENAI_API_KEY。"
...
背景元素：窗帘纹理均匀，座椅布套花纹细节锐利（色彩柔和）。整体在均匀光线下，皮肤、发丝、衣物、配饰的质感高度还原，呈现真实细节。"""
            ...
            models = []
            for model in [IMAGE_ANALYSIS_MODEL, *IMAGE_ANALYSIS_FALLBACK_MODELS]:
            ...
            for model in models:
                try:
                    ...
                    last_error = e
                    logging.warning('图片反推模型 %s 调用失败，尝试下一个 fallback: %s', model, e)

            if response is None:
                raise last_error or RuntimeError('没有可用的图片反推模型')
            ...
            if response and response.choices and len(response.choices) > 0:
                ...
                if content:
                    ...
                    return False, "未获取到图片反推结果，请重试。"
            else:
                return False, "图片反推 API 返回格式异常。"
        except Exception as e:
            error_msg = f"图片反推时出错: {str(e)}"
