...
class PopupApp {
    ...
    async init() {
        try {
            ...
            await this.loadGenerationOptions();
    ...
    async checkServerConnection() {
        try {
            ...
            if (isConnected) {
                this.uiManager.setServerStatus(true);
            } else {
                this.uiManager.setServerStatus(false);
    ...
    async loadGenerationOptions() {
        try {
            const options = await this.apiManager.getModelscopeOptions();
            await this.uiManager.applyGenerationOptions(options);
        } catch (error) {
            this.uiManager.showToast('生成配置加载失败，将使用默认设置', 'warning');
        }
    }
    ...
    async handleAnalyze() {
        ...
        if (!settings.modelscopeApiKey) {
            this.uiManager.showToast(CONFIG.ERRORS.MISSING_MODELSCOPE_TOKEN, 'error');
            this.uiManager.showSettings();
            return;
        }
