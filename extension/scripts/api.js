...
class APIManager {
    ...
    /**
     * 获取后端暴露的模型、LoRA 和默认参数
     * @returns {Promise<Object>} - ModelScope 配置
     */
    async getModelscopeOptions() {
        const url = `${this.baseUrl}${CONFIG.API.ENDPOINTS.OPTIONS}`;
    ...
    async analyzeImage(filename, settings) {
        ...
        const requestData = {
            filename: filename,
            analysis_api_key: settings.openaiKey || settings.modelscopeApiKey,
    ...
    async generateImage(prompt, settings) {
        const url = `${this.baseUrl}${CONFIG.API.ENDPOINTS.SUBMIT}`;

        const requestData = {
            prompt: prompt,
            modelscope_api_key: settings.modelscopeApiKey,
            model: settings.modelscopeModel || CONFIG.DEFAULTS.MODELSCOPE_MODEL,
            width: settings.imageWidth,
            height: settings.imageHeight,
            loras: settings.selectedLoras || CONFIG.DEFAULTS.LORAS,
            num_images: settings.numImages || CONFIG.DEFAULTS.NUM_IMAGES
        };
    ...
    async getResult(taskId) {
        return await this.getQueueStatus(taskId);
    }
            ...
            try {
                ...
                switch (status.status) {
                    case CONFIG.STATUS.COMPLETED:
                        ...
                        if (onComplete) {
                            onComplete(result.images ? result : status);
                        }
                        break;
    ...
    async processImage(file, settings, callbacks = {}) {
        ...
        try {
            ...
            if (Array.isArray(generateResult.images) && generateResult.images.length > 0) {
                const completed = {
                    ...generateResult,
                    status: CONFIG.STATUS.COMPLETED,
                    progress: 100,
                    message: CONFIG.SUCCESS.GENERATE_COMPLETE
                };
                if (onGenerateProgress) {
                    onGenerateProgress(completed);
                }
                if (onGenerateComplete) {
                    onGenerateComplete(completed);
                }
                return completed;
            }

            const taskId = generateResult.task_id || generateResult.taskId;
            if (!taskId) {
